<?php
	class LinkModel{
		...
		function prepare_link($link, $domain)
		{
			$link = rtrim($link,"/");
			foreach($domain as $d)
			{
		...
		{	
			...
			$has_child = 0;
			if(count($this->related_links))
			{
				foreach($this->related_links as $link)
				{	
					$fields = array("LINK", "SITEMAP");
					$link = $this->prepare_link($link, $domain);
			...
			self::$db->update("LINKS",array("GOT_RELATIVE_LINKS","HAS_CHILD_LINKS"),array(1,$has_child),"LINK_ID = ".$this->parent_id);
		...
		static function get_links($page, $sitemap = null)
		{
			...
			if(!empty($sitemap))
			{
				$where = "l.SITEMAP = ".$sitemap." AND ";
			}
			$where .= "lp.PAGE_ID ".(!empty($page)?(" = ".$page):"IS NULL");
